package spark.components.supportClasses
{
   ...
   public class Range extends SkinnableComponent
   {
      ...
      protected function nearestValidValue(param1:Number, param2:Number) : Number
      {
         ...
         var _loc4_:Number = 1;
         var _loc5_:Number = this.minimum;
         if(param2 != Math.round(param2))
         {
            _loc9_ = new String(1 + param2).split(".");
            _loc4_ = Math.pow(10,_loc9_[1].length);
            _loc3_ = _loc3_ * _loc4_;
            _loc5_ = _loc5_ * _loc4_;
            param2 = Math.round(param2 * _loc4_);
            param1 = Math.round(param1 * _loc4_ - _loc5_);
         }
         else
         {
            param1 = param1 - _loc5_;
         }
         var _loc6_:Number = Math.max(0,Math.floor(param1 / param2) * param2);
         var _loc7_:Number = Math.min(_loc3_,Math.floor((param1 + param2) / param2) * param2);
         var _loc8_:Number = param1 - _loc6_ >= (_loc7_ - _loc6_) / 2?Number(_loc7_):Number(_loc6_);
         return (_loc8_ + _loc5_) / _loc4_;
      }
