package spark.components.supportClasses
{
   ...
   public class ScrollBarBase extends TrackBase
   {
      ...
      private function viewport_propertyChangeHandler(param1:PropertyChangeEvent) : void
      {
         ...
         {
            case "contentWidth":
               this.viewportContentWidthChangeHandler(param1);
               break;
            case "contentHeight":
               this.viewportContentHeightChangeHandler(param1);
               break;
            case "horizontalScrollPosition":
               this.viewportHorizontalScrollPositionChangeHandler(param1);
               break;
            case "verticalScrollPosition":
               this.viewportVerticalScrollPositionChangeHandler(param1);
         }
      }
      ...
      protected function button_buttonDownHandler(param1:Event) : void
      {
         ...
         if(!this.steppingDown && !this.steppingUp)
         {
            ...
            {
               this.animateStepping(!!_loc2_?Number(maximum):Number(minimum),Math.max(this.pageSize / 10,stepSize));
            }
            return;
         }
      }
      ...
      override protected function track_mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         if(param1.shiftKey)
         {
            _loc3_ = !!thumb?Number(thumb.getLayoutBoundsWidth()):Number(0);
            _loc4_ = !!thumb?Number(thumb.getLayoutBoundsHeight()):Number(0);
            this.trackPosition.x = this.trackPosition.x - _loc3_ / 2;
            this.trackPosition.y = this.trackPosition.y - _loc4_ / 2;
         }
      ...
      {
         var _loc5_:IEaser = null;
         this.steppingDown = param1 > this.pendingValue;
         this.steppingUp = !this.steppingDown;
         var _loc3_:Number = param2 != 0?Number(param2):Number(1);
         var _loc4_:Number = getStyle("repeatInterval") * (Math.abs(param1 - this.pendingValue) / _loc3_);
         if(_loc4_ > 5000)
         {
      ...
      private function trackScrollTimerHandler(param1:Event) : void
      {
         ...
         if(getStyle("smoothScrolling"))
         {
            ...
            if(_loc2_ > value)
            {
               _loc7_ = this.pageSize != 0?int(int(_loc6_ / this.pageSize)):int(_loc6_);
            ...
            {
               _loc7_ = this.pageSize != 0?int(int(Math.ceil(_loc6_ / this.pageSize))):int(_loc6_);
