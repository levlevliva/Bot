package spark.components.supportClasses
{
   ...
   public class LabelAndIconLayout extends LayoutBase
   {
      ...
      override public function measure() : void
      {
         ...
         if(_loc4_)
         {
            _loc2_ = _loc2_ + (_loc7_ + _loc6_);
            if(_loc7_ && _loc6_)
            {
               _loc2_ = _loc2_ + this._gap;
            }
            _loc3_ = _loc3_ + Math.max(_loc8_,_loc5_);
         }
         else
         {
            _loc2_ = _loc2_ + Math.max(_loc7_,_loc6_);
            _loc3_ = _loc3_ + (_loc8_ + _loc5_);
            if(_loc8_ && _loc5_)
            {
               _loc3_ = _loc3_ + this._gap;
            }
         }
      ...
      override public function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         var _loc10_:Boolean = this.labelElement && IDisplayText(this.labelElement).text;
         var _loc11_:Number = !!_loc10_?Number(this.labelElement.getPreferredBoundsWidth()):Number(0);
         ...
         if(_loc7_)
         {
            ...
            {
               _loc14_ = _loc14_ + (_loc9_ + _loc6_);
               _loc16_ = _loc14_ - (_loc9_ + _loc6_);
            }
            else
            {
         ...
         else
         {
            ...
            _loc16_ = _loc16_ + ((_loc18_ - _loc9_) / 2 + this._paddingLeft);
            if(this.iconPlacement == IconPlacement.BOTTOM)
            {
               _loc15_ = _loc15_ + ((_loc19_ - _loc12_ - _loc8_ - _loc6_) / 2 + _loc13_ + this._paddingTop);
               _loc17_ = _loc17_ + (_loc15_ + _loc12_ + _loc6_);
            }
            else
            {
               _loc17_ = (_loc19_ - _loc12_ - _loc8_ - _loc6_) / 2 + this._paddingTop;
               _loc15_ = _loc15_ + (_loc17_ + _loc8_ + _loc6_ + _loc13_);
            }
      ...
      {
         var _loc2_:ILayoutElement = null;
         var _loc1_:int = 0;
