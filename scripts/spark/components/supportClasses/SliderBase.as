   ...
   public class SliderBase extends TrackBase implements IFocusManagerComponent
   {
      ...
      private function formatDataTipText(param1:Number) : Object
      {
         ...
         else
         {
            ...
            this.dataFormatter.fractionalDigits = this.dataTipPrecision;
            this.dataFormatter.trailingZeros = true;
      ...
      private function handleMousePoint(param1:Point) : void
      {
         ...
         if(this.dataTipInstance && this.showDataTip)
         {
      ...
      override protected function system_mouseMoveHandler(param1:MouseEvent) : void
      {
         ...
         if(!this.dragTimer)
         {
            this.dragTimer = new Timer(1000 / this.maxDragRate,0);
      ...
      override protected function keyDownHandler(param1:KeyboardEvent) : void
      {
         ...
         var _loc4_:uint = mapKeycodeForLayoutDirection(param1,true);
         switch(_loc4_)
         {
            case Keyboard.DOWN:
            case Keyboard.LEFT:
      ...
      override protected function track_mouseDownHandler(param1:MouseEvent) : void
      {
         ...
         var _loc9_:AnimationTarget = null;
         if(!enabled)
         {
            return;
         }
         var _loc2_:Number = !!thumb?Number(thumb.width):Number(0);
         var _loc3_:Number = !!thumb?Number(thumb.height):Number(0);
         ...
         _loc7_ = nearestValidValue(_loc7_,snapInterval);
         if(_loc7_ != this.pendingValue)
         {
            _loc8_ = getStyle("slideDuration");
            if(_loc8_ != 0)
            {
               if(!this.animator)
               {
                  this.animator = new Animation();
                  _loc9_ = new AnimationTarget(this.animationUpdateHandler);
                  _loc9_.endFunction = this.animationEndHandler;
