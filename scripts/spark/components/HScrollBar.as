package spark.components
{
   ...
   public class HScrollBar extends ScrollBarBase
   {
      ...
      private function updateMaximumAndPageSize() : void
      {
         ...
         var _loc2_:Number = !!isNaN(viewport.width)?Number(0):Number(viewport.width);
      ...
      override public function set viewport(param1:IViewport) : void
      {
         ...
         if(param1)
         {
            ...
            param1.addEventListener(MouseEvent.MOUSE_WHEEL,this.mouseWheelHandler,false,EventPriority.DEFAULT_HANDLER);
      ...
      override protected function pointToValue(param1:Number, param2:Number) : Number
      {
         ...
         var _loc3_:Number = track.getLayoutBoundsWidth() - thumb.getLayoutBoundsWidth();
      ...
      override protected function updateSkinDisplayList() : void
      {
         ...
         var _loc1_:Number = track.getLayoutBoundsWidth();
         if(getStyle("interactionMode") == InteractionMode.TOUCH && viewport)
         {
            _loc10_ = !!isNaN(viewport.width)?Number(0):Number(viewport.width);
         ...
         if(_loc4_ > 0)
         {
            if(!_loc5_)
            {
               ...
               _loc9_ = Math.max(thumb.minWidth,_loc9_);
            }
            else
            {
               _loc9_ = !!thumb?Number(thumb.width):Number(0);
            }
         ...
         if(pendingValue < _loc2_)
         {
            if(!_loc5_)
            {
               _loc9_ = Math.max(Math.min(thumb.minWidth,_loc9_),_loc9_ + (pendingValue - _loc2_));
            }
            _loc7_ = 0;
         }
         if(pendingValue > _loc3_)
         {
            if(!_loc5_)
            {
               _loc9_ = Math.max(Math.min(thumb.minWidth,_loc9_),_loc9_ - (pendingValue - _loc3_));
            }
            _loc7_ = _loc1_ - _loc9_;
         }
         if(!_loc5_)
         {
            thumb.setLayoutBoundsSize(_loc9_,NaN);
         }
         if(getStyle("autoThumbVisibility") === true)
         {
            thumb.visible = _loc9_ < _loc1_;
         }
         _loc6_ = track.localToGlobal(new Point(_loc7_,0));
         if(thumb.parent)
         {
            _loc8_ = thumb.parent.globalToLocal(_loc6_).x;
         }
         thumb.setLayoutBoundsPosition(Math.round(_loc8_),thumb.getLayoutBoundsY());
      }
      ...
      override public function changeValueByPage(param1:Boolean = true) : void
      {
         ...
         {
            _loc2_ = pageSize;
            pageSize = Math.abs(viewport.getHorizontalScrollPositionDelta(!!param1?uint(NavigationUnit.PAGE_RIGHT):uint(NavigationUnit.PAGE_LEFT)));
         }
      ...
      override protected function animatePaging(param1:Number, param2:Number) : void
      {
         ...
         if(viewport)
         {
            _loc3_ = Math.abs(viewport.getHorizontalScrollPositionDelta(param1 > value?uint(NavigationUnit.PAGE_RIGHT):uint(NavigationUnit.PAGE_LEFT)));
      ...
      override public function changeValueByStep(param1:Boolean = true) : void
      {
         ...
         {
            _loc2_ = stepSize;
            stepSize = Math.abs(viewport.getHorizontalScrollPositionDelta(!!param1?uint(NavigationUnit.RIGHT):uint(NavigationUnit.LEFT)));
         }
      ...
      override mx_internal function viewportHorizontalScrollPositionChangeHandler(param1:PropertyChangeEvent) : void
      {
         if(viewport)
         {
      ...
      override mx_internal function viewportContentWidthChangeHandler(param1:PropertyChangeEvent) : void
      {
         if(viewport)
         {
            if(getStyle("interactionMode") == InteractionMode.TOUCH)
      ...
      mx_internal function mouseWheelHandler(param1:MouseEvent) : void
      {
         ...
         while(_loc8_ < _loc5_)
         {
            _loc9_ = _loc2_.getHorizontalScrollPositionDelta(_loc6_);
            if(!isNaN(_loc9_))
            {
               _loc2_.horizontalScrollPosition = _loc2_.horizontalScrollPosition + _loc9_;
               _loc7_ = true;
      ...
      private function hsb_mouseWheelHandler(param1:MouseEvent) : void
      {
         var _loc2_:IViewport = viewport;
