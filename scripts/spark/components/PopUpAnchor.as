package spark.components
{
   ...
   public class PopUpAnchor extends UIComponent
   {
      ...
      protected function calculatePopUpPosition() : Point
      {
         ...
         this.determinePosition(this.popUpPosition,_loc5_.width,_loc5_.height,_loc2_,_loc3_,_loc4_);
         if(screen)
         {
         ...
         if(_loc6_ != null)
         {
            ...
            this.determinePosition(_loc6_,_loc5_.width,_loc5_.height,_loc2_,_loc9_,_loc10_);
            if(screen)
            {
               switch(_loc6_)
               {
         ...
         var _loc7_:Number = decomposition[3];
         var _loc8_:Number = decomposition[4];
         if(_loc4_.top < screen.top)
         {
            _loc3_.y = _loc3_.y + (screen.top - _loc4_.top) / _loc8_;
         }
         else if(_loc4_.bottom > screen.bottom)
         {
            _loc3_.y = _loc3_.y - (_loc4_.bottom - screen.bottom) / _loc8_;
         }
         if(_loc4_.left < screen.left)
         {
            _loc3_.x = _loc3_.x + (screen.left - _loc4_.left) / _loc7_;
         }
         else if(_loc4_.right > screen.right)
         {
            _loc3_.x = _loc3_.x - (_loc4_.right - screen.right) / _loc7_;
         }
         if(layoutDirection == LayoutDirection.RTL)
         {
            _loc3_.x = _loc3_.x + _loc4_.width;
         }
      ...
      protected function addOrRemovePopUp() : void
      {
         ...
         {
            PopUpManager.addPopUp(this.popUp,this,this._isModal);
      ...
      protected function determinePosition(param1:String, param2:Number, param3:Number, param4:Matrix, param5:Point, param6:Rectangle) : void
      {
         switch(param1)
         {
            case PopUpPosition.BELOW:
      ...
      protected function applyPopUpTransform(param1:Number, param2:Number) : void
      {
         ...
         {
            _loc4_ = !!this.popUpWidthMatchesAnchorWidth?Number(param1):Number(this.popUp.measuredWidth);
            _loc5_ = !!this.popUpHeightMatchesAnchorHeight?Number(param2):Number(this.popUp.measuredHeight);
            this.popUp.setActualSize(_loc4_,_loc5_);
         }
         var _loc3_:Point = this.calculatePopUpPosition();
         PopUpUtil.applyPopUpTransform(this,method_296.concatenatedColorTransform,systemManager,this.popUp,_loc3_);
      }
