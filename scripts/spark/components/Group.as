package spark.components
{
   ...
   public class Group extends GroupBase implements IVisualElementContainer, IGraphicElementContainer, ISharedDisplayObject
   {
      ...
      private var deferredStyleClients:Dictionary = null;
      ...
      override public function set blendMode(param1:String) : void
      {
         ...
         if(param1 == "auto")
         {
            ...
            if(alpha > 0 && alpha < 1 && super.method_294 != BlendMode.LAYER || (alpha == 1 || alpha == 0) && super.method_294 != BlendMode.NORMAL)
            {
               this.invalidateDisplayObjectOrdering();
            }
         }
      ...
      override public function addStyleClient(param1:IAdvancedStyleClient) : void
      {
         if(!this.createChildrenCalled)
         {
            if(!this.deferredStyleClients)
            {
               this.deferredStyleClients = new Dictionary(true);
            }
            this.deferredStyleClients[param1] = true;
         }
         else
         {
            super.addStyleClient(param1);
         }
      }
      
      override public function removeStyleClient(param1:IAdvancedStyleClient) : void
      {
         if(this.deferredStyleClients && !this.createChildrenCalled)
         {
            delete this.deferredStyleClients[param1];
         }
         else
         {
            super.removeStyleClient(param1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         var _loc2_:IAdvancedStyleClient = null;
         ...
         if(this.deferredStyleClients)
         {
            for(_loc1_ in this.deferredStyleClients)
            {
               _loc2_ = _loc1_ as IAdvancedStyleClient;
               if(_loc2_)
               {
                  super.addStyleClient(_loc2_);
               }
            }
            this.deferredStyleClients = null;
         }
      }
      ...
      override protected function commitProperties() : void
      {
         ...
         if(this.blendModeChanged)
         {
            ...
            if(this._blendMode == "auto")
            {
               if(alpha == 0 || alpha == 1)
               {
                  super.method_294 = BlendMode.NORMAL;
               }
               else
               {
                  super.method_294 = BlendMode.LAYER;
               }
            }
            else if(!this.isAIMBlendMode(this._blendMode))
            {
               super.method_294 = this._blendMode;
            }
      ...
      override public function parentChanged(param1:DisplayObjectContainer) : void
      {
         ...
         var _loc2_:Array = this.MXMLDescriptor;
         if(_loc2_ && !processedMXMLDescriptors)
         {
            generateMXMLInstances(document,_loc2_);
