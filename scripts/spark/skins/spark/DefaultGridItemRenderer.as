package spark.skins.spark
{
   ...
   public class DefaultGridItemRenderer extends UIFTETextField implements IGridItemRenderer, IStyleClient
   {
      ...
      {
         return !!this._column?int(this._column.columnIndex):-1;
      }
      ...
      private function updatePreferredSize() : void
      {
         ...
         if(!this.wordWrapSet && this.column && this.column.grid)
         {
         ...
         super.validateNow();
         var _loc1_:Number = !!isNaN(explicitWidth)?Number(textWidth + LEFT_PADDING + RIGHT_PADDING + TEXT_WIDTH_PADDING):Number(Math.max(explicitWidth,4,LEFT_PADDING + RIGHT_PADDING + TEXT_WIDTH_PADDING));
         var _loc2_:Number = !!isNaN(explicitHeight)?Number(textHeight + TOP_PADDING + BOTTOM_PADDING + TEXT_HEIGHT_PADDING):Number(Math.max(explicitHeight,TOP_PADDING + BOTTOM_PADDING + TEXT_HEIGHT_PADDING));
         if(!stage || embedFonts)
         {
