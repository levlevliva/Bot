package package_147
{
   import com.bigpoint.seafight.model.inventory.vo.ammunition.Ammo_VO;
   import com.bigpoint.seafight.tools.center;
   import com.bigpoint.seafight.view.common.skins.vscroller.VScrollBarSmallSkin;
   import com.bigpoint.seafight.view.popups.gempopup.ship.component.GemSocket;
   import com.bigpoint.seafight.view.popups.guildmenu.common.GuildMenuCurrencyController;
   import com.bigpoint.seafight.view.popups.mateysmenu.vo.PlayerProfileViewVo;
   import flash.events.Event;
   import flash.utils.IDataInput;
   import flash.utils.IDataOutput;
   import mx.binding.BindingManager;
   import mx.collections.ArrayList;
   import net.bigpoint.seafight.com.module.guild.class_433;
   import package_14.class_47;
   import package_14.class_88;
   import package_205.class_1353;
   import package_208.class_1361;
   import package_26.class_1354;
   import package_41.class_93;
   import package_51.class_1355;
   import package_54.class_319;
   import package_6.class_14;
   import package_69.class_978;
   import spark.components.VScrollBar;
   import spark.primitives.Rect;
   
   public final class class_738 implements class_14
   {
      
      public static const name_3:class_738 = new class_738();
       
      
      private var var_646:int = 0;
      
      public var var_263:Vector.<class_731>;
      
      public function class_738(param1:Vector.<class_731> = null)
      {
         super();
         if(param1 == null)
         {
            this.var_263 = new Vector.<class_731>();
         }
         else
         {
            this.var_263 = param1;
         }
      }
      
      public function method_16() : int
      {
         return 3508;
      }
      
      public function method_17() : int
      {
         return 0;
      }
      
      public function method_18() : int
      {
         return this.var_646;
      }
      
      public function method_19() : int
      {
         return 4;
      }
      
      public function method_15(param1:IDataInput) : void
      {
         var _loc4_:class_731 = null;
         this.var_646 = param1.readShort();
         this.var_646 = 65535 & ((65535 & this.var_646) >>> 6 % 16 | (65535 & this.var_646) << 16 - 6 % 16);
         this.var_646 = this.var_646 > 32767?int(this.var_646 - 65536):int(this.var_646);
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         while(this.var_263.length > 0)
         {
            this.var_263.pop();
         }
         _loc2_ = 0;
         _loc3_ = param1.readByte();
         while(_loc2_ < _loc3_)
         {
            _loc4_ = class_731(class_93.method_26().method_25(param1.readShort()));
            _loc4_.method_15(param1);
            this.var_263.push(_loc4_);
            _loc2_++;
         }
      }
      
      public function method_14(param1:IDataOutput) : void
      {
         var _loc2_:class_731 = null;
         param1.writeShort(3508);
         param1.writeShort(65535 & ((65535 & 0) << 6 % 16 | (65535 & 0) >>> 16 - 6 % 16));
         param1.writeByte(this.var_263.length);
         for each(_loc2_ in this.var_263)
         {
            _loc2_.method_14(param1);
         }
      }
   }
}
