   ...
   public class BindabilityInfo
   {
      ...
      public function getChangeEvents(param1:String) : Object
      {
         ...
         if(!changeEvents)
         {
            ...
            if(numChildren == 0)
            {
               if(!this.typeDescription.@var_2508)
               {
                  "warning: no describeType entry for \'" + childName + "\' on non-dynamic type \'" + this.typeDescription.@name + "\'";
               }
            }
            else
            {
               if(numChildren > 1)
               {
                  "warning: multiple describeType entries for \'" + childName + "\' on type \'" + this.typeDescription.@name + "\':\n" + childDesc;
               }
      ...
      private function addChangeEvents(param1:XMLList, param2:Object, param3:Boolean) : void
      {
         ...
         for each(_loc4_ in param1)
         {
            ...
            {
               "warning: unconverted Bindable metadata in class \'" + this.typeDescription.@name + "\'";
            }
         }
      }
