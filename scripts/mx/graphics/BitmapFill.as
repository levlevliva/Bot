package mx.graphics
{
   ...
   public class BitmapFill extends EventDispatcher implements IFill
   {
      ...
      {
         return !!isNaN(this.x)?Number(0):Number(this.x);
      }
      
      public function set offsetX(param1:Number) : void
      {
         var _loc2_:Number = !!isNaN(this.x)?Number(0):Number(this.x);
         this.x = param1;
      ...
      {
         return !!isNaN(this.y)?Number(0):Number(this.y);
      }
      
      public function set offsetY(param1:Number) : void
      {
         var _loc2_:Number = !!isNaN(this.y)?Number(0):Number(this.y);
         this.y = param1;
      ...
      public function set repeat(param1:Boolean) : void
      {
         ...
         if(param1 != _loc2_)
         {
            this.fillMode = !!param1?BitmapFillMode.REPEAT:BitmapFillMode.SCALE;
      ...
      {
         return !!this.compoundTransform?Number(this.compoundTransform.scaleX):Number(this._scaleX);
      }
      ...
      {
         return !!this.compoundTransform?Number(this.compoundTransform.scaleY):Number(this._scaleY);
      }
      ...
      public function set source(param1:Object) : void
      {
         ...
         if(param1 != this._source)
         {
            ...
            if(!_loc4_ && _loc2_)
            {
               ...
               _loc4_.draw(_loc2_,new Matrix());
               _loc5_ = true;
            }
            if(_loc4_ && !_loc4_.transparent && this.alpha != 1)
            {
               _loc7_ = new BitmapData(_loc4_.width,_loc4_.height,true);
               _loc7_.draw(_loc4_);
               _loc5_ = true;
               _loc4_ = _loc7_;
            }
      ...
      {
         return !!this.compoundTransform?Number(this.compoundTransform.transformX):Number(this._transformX);
      }
      ...
      {
         return !!this.compoundTransform?Number(this.compoundTransform.transformY):Number(this._transformY);
      }
      ...
      public function begin(param1:Graphics, param2:Rectangle, param3:Point) : void
      {
         ...
         var _loc4_:BitmapData = this._bitmapData;
         if(!_loc4_)
         {
            return;
         }
         var _loc5_:* = this.fillMode == BitmapFillMode.REPEAT;
         if(this.nonRepeatAlphaSource && this.applyAlphaMultiplier)
         {
         ...
         else
         {
            ...
            _loc8_ = !isNaN(this.x)?Number(this.x + param3.x):Number(param2.left);
            _loc9_ = !isNaN(this.y)?Number(this.y + param3.y):Number(param2.top);
         ...
         {
            if(this.nonRepeatAlphaSource && this.nonRepeatSourceCreated)
            {
               this.nonRepeatAlphaSource.dispose();
               this.nonRepeatAlphaSource = null;
         ...
         else if(this.fillMode == BitmapFillMode.CLIP)
         {
            if(this.regenerateNonRepeatSource || this.lastBoundsWidth != param2.width || this.lastBoundsHeight != param2.height)
            {
               ...
               _loc10_ = new Point();
               _loc11_ = transformMatrix.tx;
               _loc12_ = transformMatrix.ty;
         ...
         if(this.applyAlphaMultiplier)
         {
            ...
            _loc16_.alphaMultiplier = this.alpha;
