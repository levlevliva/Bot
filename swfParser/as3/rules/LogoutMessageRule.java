 ...
 */
public class LogoutMessageRule extends Rule {
    ...
    public void process(AS3Parser context) {
        ...
        String[] result = context.findFileByString(".BUTTON_LOGOUT:");
        String searchSrc = result[0];

        int lineIndex = context.findLineIndexByString(searchSrc,".BUTTON_LOGOUT:");
        String line = context.findLineByIndex(searchSrc,lineIndex+1);
        String class_name = RegexHelper.match(line,"\\((class_.*?)\\..*?\\);")[1];
    ...
    public void export() {
        try{
            Files.write(Paths.get("logout_message.json"),jsonString.getBytes("UTF8"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
